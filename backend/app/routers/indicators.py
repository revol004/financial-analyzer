...
def update_indicator(indicator_id: int, indicator: IndicatorUpdate, db: Session = Depends(get_db)):
    ...
    for year in request.years:
        ...
        for indicator in indicators:
            if indicator.agg_type in ('median', 'mean'):
                ...
                base_ind = base_indicators.get(indicator.base_indicator_id) if indicator.base_indicator_id else None
                count = 0
                ...
                for y in range(year - agg_years + 1, year + 1):
                    ...
                    if base_ind:
                        ...
                        raw_q = db.query(FinancialData).filter(
                            FinancialData.company_id == request.company_id,
                            FinancialData.year == y,
                            FinancialData.quarter.is_(None),
                    ...
                    if val is not None:
                        try:
                            if eval(f"{val} {' '.join(condition.split()[1:])}", {"__builtins__": {}}):
                                count += 1
                        except Exception:
                            pass
